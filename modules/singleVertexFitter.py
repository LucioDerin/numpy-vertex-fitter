...
class singleVertexFitter_straightTracks:
    ...
    def __init__(self, eps = 1e-8, maxIter = 1e2) -> None:
        """Constructor of the fitter.

        Args:
            eps (double, optional): stability tolerance. Defaults to 1e-8.
            maxIter (int, optional): maximum number of minimization iteration. Defaults to 1e2.
        """
        self.eps = eps
    ...
    def fit(self, tracks):
        """Functions that fit a single vertex on the tracks (H5Tracks) contained in the given list, assumed to be straight tracks.

        Args:
            tracks (list of H5Tracks): list of tracks to be fitted.

        Returns:
            np.array: shape (3,), coordinates of the fitted vertex [z,x,y].
        """
        ...
        while iter < self.maxIter:
            ...
            for i,t in enumerate(tracks):
                ai = t.versor
                Hi = np.diag([ai[1]**2 + ai[2]**2, ai[0]**2 + ai[2]**2, ai[0]**2 + ai[1]**2])
